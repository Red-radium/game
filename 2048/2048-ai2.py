    ...
    if layer == 0:
        ...
        for i,direction in enumerate("wasd"):
            ...
            if grid.grid != grid.prev:
                points[i] = how_good_search(grid,layer)
        else: points[i] = -1
        return max(points)
class grid(object):
    ...
    def ai(self):
        ...
        for i,direction in enumerate("wasd"):
            ...
            if self.grid != self.prev:
                points[i] = how_good_search(self,2)
                #print(self.grid)
            else: points[i] = -1
...
class world(object):
    ...
    def get_direction_move(self):
        ...
        if event.type == pygame.KEYDOWN: direction = self.grid.ai()

        #manual
        # keys =pygame.key.get_pressed()
        # if event.key == pygame.K_w: direction = "w"
        # elif event.key == pygame.K_a: direction = "a"
        # elif event.key == pygame.K_s: direction = "s"
        ...
        for y,i in enumerate(self.grid.grid):
            for x,j in enumerate(i):
                if j != None:
                    ...
                    try:
                        text = FONT.render(str(j),True,((j/BASE**13)*255,0,0))
                    except:
